...
def apply_freq_transform(img_rgb: np.ndarray, B_mask: np.ndarray, mode: str,
    ...
    for ch in range(3):
        ...
        if mode == "A":
            # 저주파 진폭 제거 (단, DC 성분(전체 평균 밝기)은 보존)
            # DC까지 제거하면 이미지 전체가 거의 검은색이 되어
            # ControlNet 조건으로서 정보가 거의 남지 않음 (학습 난이도 과도하게 높아짐)
            mask_no_dc = B_mask.copy()
            cy, cx = B_mask.shape[0] // 2, B_mask.shape[1] // 2
            mask_no_dc[cy, cx] = 0.0
            amp_new = amp * (1.0 - mask_no_dc)
        elif mode == "B":
